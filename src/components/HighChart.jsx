...
import { getyyMMFromDate } from '../common/commonFunc';
...
const HighChart = React.memo(({data, dataName, settingVal1, settingVal2}) => {
  ...
  let dataArr_All_Month = chartFunc.changeDateToMonth(dataArr_All); //코스피 월간 전체 데이터 (2001 ~ 현재)
   ...
   // 메인 화면에서는 현재 -6년 이후 데이터만 보여줄것 
  const thisYear = getyyMMFromDate(new Date()).substring(0,4)*1;
  
  const cutYear = thisYear-6;
  ...
  // 횡보구간 레인지 추가
  if(sideWalkArr.length!==0){
    const sideWalkSeriesArr = chartSeries.getSideWalkSeries(sideWalkArr);
    sideWalkSeriesArr.length!==0 && options.series.push(sideWalkSeriesArr);
  ...
  const maSeriesArr = chartSeries.getMovingAverageSeries(ma3Arr, ma12Arr, ma20Arr, ma60Arr, ma120Arr, 'main');
